...
export function useToast() {
  ...
  useEffect(() => {
    ...
    const unsub = window.electronAPI.onError((error) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 space-y-2 max-w-sm">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className={`px-4 py-3 rounded-2xl backdrop-blur-2xl border text-sm font-medium shadow-2xl animate-in slide-in-from-right ${
            toast.type === 'error'
              ? 'bg-red-500/15 border-red-500/20 text-red-300'
              : toast.type === 'success'
                ? 'bg-green-500/15 border-green-500/20 text-green-300'
                : 'bg-white/10 border-white/[0.06] text-white/80'
          }`}
        >
          {toast.text}
        </div>
